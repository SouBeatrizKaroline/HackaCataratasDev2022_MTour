// variables
let words = ['cataratas do iguacu', 'pira de Foz', 'foz do iguacu', 'shawarma', 'medianeira', 'matelandia', 'macuco safari', 'itaipu', 'mboi', 'canoagem'];
let chosenWord;
let correctWord = "";
let letters = [];
let tries = 10;
...
function checkGameOver() {
    if (tries == 0) {
        // gameover
        showEndGameText("lose", "Não desista! Foz do Iguaçu é lindissima, se tiver a chance vá conhece-la.");
        gameOver = true;
    }
    if (correctWord.length === chosenWord.length) {
        // gamewin
        showEndGameText("win", "Parabéns! Já pode se considerar um iguaçuense!");
        gameOver = true;
    }
}
